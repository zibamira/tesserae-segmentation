...
HxLineRaycast::initForLineSetInterface(HxLineSetInterface* newLineSet)
{
    ...
    QStringList rad_labels;
    QStringList col_labels;
    McDArray<QString> labels(numDataValues + 2);

    labels[0] = "none";
    labels[1] = "constant";

    rad_labels.append(labels[1]);
    col_labels.append(labels[0]);
    col_labels.append(labels[1]);

    for (int i = 0; i < numDataValues; ++i)
    {
        labels[i + 2] = QString("data value %1").arg(i);
        rad_labels.append(labels[i + 2]);
        col_labels.append(labels[i + 2]);
    }

    mPortLineRadiusData.insertComboBox(0, rad_labels);
    mPortLineColorStyle.insertComboBox(0, col_labels);

    mPortEndingRadiusData.insertCheckBox(0, "adapt to lines");
    mPortEndingRadiusData.insertComboBox(1, rad_labels);
    mPortEndingRadiusData.setValue(0, adaptEndingRad);
    mPortEndingColorStyle.insertCheckBox(0, "adapt to lines");
    mPortEndingColorStyle.insertComboBox(1, col_labels);
    mPortEndingColorStyle.setValue(0, adaptEndingCol);

    mPortNodeRadiusData.insertComboBox(0, rad_labels);
    mPortNodeColorStyle.insertCheckBox(0, "adapt to lines");
    mPortNodeColorStyle.insertComboBox(1, col_labels);
...
HxLineRaycast::updateEndingColorPorts()
{
    ...
    {
        // handling adaption: only true for linesets
        ...
        else if (mPortDisplay.getValue(1))
        {
            ...
            // line set and att
            if (mLineSetInterface && att == 1)
                mPortEndingColorConstant.show();
            else if (mLineSetInterface && att > 1)
                mPortEndingColorMap.show();

            // spatial graph and att
            if (mSpatialGraphInterface && att == 1)
                mPortEndingColorConstant.show();
            else if (mSpatialGraphInterface && att > 1 && mSpatialGraphInterface->hasLabel(mEndingColorAtt[att - 2].mAttribute))
            {
...
HxLineRaycast::updateLineColorPorts()
{
    if (mPortLineColorStyle.isNew())
    {
        ...
        if (mSpatialGraphInterface)
        {
            if (att == 1)
                mPortLineColorConstant.show();
            else if ((att - 2 < ns && att > 1 && mSpatialGraphInterface->hasLabel(mSegmentColorAtt[att - 2].mAttribute)) ||
                     (att - 2 >= ns && att > 1 && mSpatialGraphInterface->hasLabel(mNodeColorAtt[att - 2 - ns].mAttribute)))
            {
...
HxLineRaycast::updateNodeColorPorts()
{
    ...
    if (mPortNodeColorStyle.isNew())
    {
        ...
        if (mSpatialGraphInterface)
        {
            ...
            {
                if ((att - 2 < ns && mSpatialGraphInterface->hasLabel(mSegmentColorAtt[att - 2].mAttribute)) ||
                    (att - 2 >= ns && mSpatialGraphInterface->hasLabel(mNodeColorAtt[att - 2 - ns].mAttribute)))
                {
