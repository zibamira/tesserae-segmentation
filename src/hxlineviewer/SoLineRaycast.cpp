    ...
    , mParamSegmentNormalInterpolation(1.f)
{
    ...
    McString shaderDir(HxResource::getRootDir());
    shaderDir += "/share/shaders/hxlineviewer/Cylinder";
    mTargets[SEGMENT].mShader.load(shaderDir.getString(), 4, GL_POINTS, GL_TRIANGLE_STRIP);

    shaderDir = HxResource::getRootDir();
    shaderDir += "/share/shaders/hxlineviewer/Sphere";
    mSegmentSphereShader.load(shaderDir.getString(), 4, GL_POINTS, GL_TRIANGLE_STRIP);

    shaderDir = HxResource::getRootDir();
    shaderDir += "/share/shaders/hxlineviewer/Sphere";
    mTargets[ENDING].mShader.load(shaderDir.getString(), 4, GL_POINTS, GL_TRIANGLE_STRIP);

    shaderDir = HxResource::getRootDir();
    shaderDir += "/share/shaders/hxlineviewer/Sphere";
    mTargets[NODE].mShader.load(shaderDir.getString(), 4, GL_POINTS, GL_TRIANGLE_STRIP);
}
...
int
SoLineRaycast::getRadiusData(const SoLineRaycast::Target& target) const
{
...
SoLineRaycast::mouseClick(SoEventCallback* eventCB)
{
    ...
    {
        bool ctrl = (bool)event->wasCtrlDown();

        const bool control = (bool)event->wasCtrlDown();
        if (!control)
            mSelectedEndingIDs.clear();
        ...
        if (eventCB->getPickedPoint())
        {
            ...
            if (pickDetail && pickDetail->getTypeId() == SoPointDetail::getClassTypeId())
            {
                ...
                for (int i = 0; i < mSelectedEndingIDs.size(); ++i)
                {
                    if (mSelectedEndingIDs[i] == pointDetail->getCoordinateIndex())
                    {
                        sel = i;
                        break;
                    }
                ...
                if (sel >= 0)
                {
                    mSelectedEndingIDs.remove(sel);
                }
                else
                {
                    mSelectedEndingIDs.push(pointDetail->getCoordinateIndex());
                }
            }
...
const McDArray<int>&
SoLineRaycast::getSelected() const
{
    return mSelectedEndingIDs;
}

void
SoLineRaycast::setSelection(const McDArray<int>& selected)
{
    clearSelection();
    mSelectedEndingIDs = selected;
...
SoLineRaycast::updateSelection()
{
    ...
    for (int i = 0; i < mSelectedEndingIDs.size(); ++i)
    {
        mEnding[mSelectedEndingIDs[i]].mSelected = 1.f;
    }
...
SoLineRaycast::updateNodesFromLineInterface()
{
    ...
    int numNodes = mLineSetInterface->getNumPoints();
    int colData = tar.mColorData - 1;
    int radData = tar.mRadiusData;
...
SoLineRaycast::updateShader(Target target)
{
    ...
    if (target == SEGMENT)
    {
        McString shaderDir2(HxResource::getRootDir());
