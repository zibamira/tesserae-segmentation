...
public:
    ...
    int getRadiusData(const Target& target) const;

    /// Returns selected elements.
    const McDArray<int>& getSelected() const;

    void mouseClick(SoEventCallback* eventCB);

    /// Set the selected elements without raycasting.
    void setSelection(const McDArray<int>& selected);
...
private:
    ...
    McDArray<int> mSelectedEndingIDs;
