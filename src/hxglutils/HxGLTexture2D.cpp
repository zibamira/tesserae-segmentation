...
void HxGLTexture2D::GLTexture2D::updateResource(void* data)
{
...
void HxGLTexture2D::activate(unsigned int contextID)
{
...
void HxGLTexture2D::bind(unsigned int contextID, HxGLShader& shader, int texunit, const char* name)
{
    mTexture.update(contextID, &mData);
    ...
    if (set3D) glDisable(GL_TEXTURE_3D);
    ...
    if (set3D) glEnable(GL_TEXTURE_3D);
}




void HxGLTexture2D::copyColorFrameBuffer(unsigned int contextID)
{
...
void HxGLTexture2D::copyDepthFrameBuffer(unsigned int contextID)
{
    mData.mInternalFormat = GL_DEPTH_COMPONENT;
    mData.mFormat         = GL_DEPTH_COMPONENT24;
...
GLuint HxGLTexture2D::getTexture(unsigned int contextID, bool update)
{
...
void HxGLTexture2D::setFetchMode(GLint mode)
{
...
void HxGLTexture2D::setFormat(GLint internalFormat, GLenum format, GLenum type)
{
...
void HxGLTexture2D::setWrap(GLint param)
{
    mData.mWrap_S = param;
    mData.mWrap_T = param;
...
void HxGLTexture2D::setSize(GLsizei width, GLsizei height)
{
...
void HxGLTexture2D::setTexture(int size, void *data)
{
    mData.mWidth  = size;
    mData.mHeight = size;
    mData.mData   = data;
...
void HxGLTexture2D::setTextureData(void *data)
{
    mData.mData   = data;
...
void HxGLTexture2D::unbind(int texunit)
{
    bool set3D      = glIsEnabled(GL_TEXTURE_3D);
    GLint activeTex = GL_TEXTURE0;
...
int HxGLTexture2D::getWidth() const {
    return mData.mWidth;
}

int HxGLTexture2D::getHeight() const {
